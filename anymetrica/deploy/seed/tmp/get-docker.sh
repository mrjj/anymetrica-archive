...
do_install() {
	...
	if command_exists docker; then
		...
		EOF
		( set -x; sleep ${WARNING_SLEEP_TIME_SEC} )
	fi

	user="$(id -un 2>/deploy-dev/null || true)"
	...
	case "$lsb_dist" in
		ubuntu|debian|raspbian)
			...
			(
				...
				$sh_c 'apt-get update -qq >/deploy-dev/null'
				$sh_c "apt-get install -y -qq $pre_reqs >/deploy-dev/null"
				$sh_c "curl -fsSL \"$DOWNLOAD_URL/linux/$lsb_dist/gpg\" | apt-key add -qq - >/deploy-dev/null"
			...
			(
				...
				$sh_c "apt-get install -y -qq --no-install-recommends docker-ce$pkg_version >/deploy-dev/null"
			)
			echo_docker_as_nonroot
			exit 0
			;;
