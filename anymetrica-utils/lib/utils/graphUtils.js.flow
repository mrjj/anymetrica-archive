...
const toposort = (
  tasksToSort: Array<any>,
  cachedTasksMap: Object,
  getUpstream: (Object) => any = task => task.input(),
) => {
  const lookupById = cachedTasksMap || {};
  tasksToSort.forEach((t: any) => {
    lookupById[t.id] = t;
  });
  const topologicalSortHelper = (task, visited, temp, tts: Array<any>, result) => {
    temp[task.id] = true;
 ...
 */
const matrixToGraph = (matrixData: MdInputType) => {
  const links = [];
